...
import {Chatbubble, People, QrCodeOutline} from "react-ionicons";
          ...
          <div className="flex flex-col ml-3 w-full">
            <div className="flex flex-row items-center">
              <div className="flex flex-col">
                ...
                <Text type="text-4" className="text-dp-gray">{account.slug}</Text>
              </div>
              <div className="flex flex-row ml-auto">
                <Text type="text-2" className="text-dp-purple">{account.shared}</Text>
  ...
  return (
    <div className="flex flex-col flex-grow flex-start content-start items-start justify-start">
      ...
      <section className="flex flex-col w-full justify-center items-center">
        ...
        <Text type="text-4" className="text-dp-gray-dark">deohge</Text>
        <Text type="text-3" className="text-dp-gray-dark mt-3 w-4/5 text-center">I am Dashy and this is my status
          message</Text>
      </section>

      <section className="controls flex justify-center space-x-6 w-full my-4">
        <div className="w-[2.5rem] h-[2.5rem] bg-dp-gray-light rounded-full flex items-center justify-center">
          <Chatbubble height="1.2rem" width="1.2rem" color="rgb(var(--purple))" />
        </div>
        <div className="w-[2.5rem] h-[2.5rem] bg-dp-gray-light rounded-full flex items-center justify-center">
          <DashSvg fill="rgb(var(--purple))" className="scale-[0.9]"/>
        </div>
        <div className="w-[2.5rem] h-[2.5rem] bg-dp-gray-light rounded-full flex items-center justify-center">
          <QrCodeOutline height="1.2rem" width="1.2rem" color="rgb(var(--purple))" />
        </div>
      </section>

      <section className="user-info w-full mb-3">
        <div className="w-1/2 flex float-left items-center justify-center">
          <DashSvg fill="rgb(var(--purple))" className="scale-[0.8]"/>
          <Text type="text-5" className="ml-1 text-dp-gray-dark">Joined May 2021</Text>
        </div>
        <div className="flex w-1/2 float-left items-center justify-center">
          <People height="1.2rem" width="1.2rem" color="rgb(var(--purple))" />
          <Text type="text-5" className="ml-1 text-dp-gray-dark">Friends since June 2021</Text>
        </div>
      </section>

      <section className="w-full">
        <SearchInput placeholder="Search for users" value={state.search} onChange={onSearch}/>
        <div className="tabs w-full flex flex-row mt-2">
          <div className="tab flex-grow">
            <Text type="text-2" className="tab-name text-center text-dp-purple pt-1 pb-3">Friends</Text>
            <div className="tab-underline h-[4px] -mt-[4px] bg-dp-purple rounded"></div>
          </div>
          <div className="tab flex-grow">
            <Text type="text-2" className="tab-name text-center text-dp-purple py-1">Shared Friends</Text>
          </div>
        </div>
