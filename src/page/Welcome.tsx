...
export default function(props: any) {
  ...
  const onNext = () => {
    if (state.activeSlide == slides.length - 1) nav('/username')
  ...
  const cls = {
    selector: {
      active: 'w-[8px] h-[8px] bg-dp-bg-purple',
    ...
    <div className="dp-welcome flex flex-grow flex-col">
      {slides.filter((_, i) => i == state.activeSlide).map((slide, k) => (
        <div key={k} className="dp-welcome-slide flex flex-col items-center">
          ...
          <span className="text-base font-light text-dp-text-gray text-center mt-2 leading-tight max-w-[85%]">{slide.text}</span>
        </div>
      ))}
      ...
      {state.activeSlide == slides.length - 1 && (
        <div className="flex justify-center items-center mt-3">
          <span className="text-sm text-dp-text-gray font-light">Already have an account?</span>
          <Link className="text-sm text-dp-text-purple font-semibold ml-1" to="/accounts">Login</Link>
        </div>
      )}
    </div>
  )
